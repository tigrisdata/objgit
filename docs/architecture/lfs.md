...
Every LFS handler caps the body it reads with `http.MaxBytesReader`. A body over
the cap gets a 413. These routes answer an anonymous caller when `-allow-lfs` is
set, so the daemon must not hold an arbitrary amount of JSON.

The cap comes from `-lfs-max-batch`, and not from a flag of its own. One object
entry is an oid, a size, and punctuation, so the batch cap sets the size of the
largest legal body.

The verify and lock routes accept an empty body, which the protocol permits.
Those handlers refuse an oversize body, and they tolerate every other decode
error.
...
Two caps bound the document. One path is 4096 bytes at most, which is PATH_MAX
on Linux and more than git can check out. A repository holds 1000 locks at most,
which is far above the handful of binary assets that file locking coordinates. A
request over the path cap gets a 422. A request over the count cap gets a 413.

The count cap is soft. Two creates that race can both pass it. The cap bounds
growth over time, and it is not an exact limit. A path that already holds a lock
passes the count cap, so a repeated `git lfs lock` still gets the 409 that names
the holder.
